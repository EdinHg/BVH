...
// Compile: nvcc -std=c++14 -arch=sm_75 -O3 cudaLBVH.cu -o cudaLBVH
// For Colab: nvcc -std=c++14 -arch=sm_75 -O3 --expt-relaxed-constexpr cudaLBVH.cu -o cudaLBVH
// Usage:
//   Random triangles: ./cudaLBVH <num_triangles> [--export] [--export-leaves]
//   From OBJ file:    ./cudaLBVH <model.obj> [--export] [--export-leaves]
...
// Compute triangle bounds and centroids
...
__global__ void reduceBounds(const AABB* triBounds, AABB* output, size_t n) {
    __shared__ AABB sharedBounds[256];
    ...
    AABB localBounds;
    if (gid < n) {
        localBounds = triBounds[gid];
    }

    sharedBounds[tid] = localBounds;
    __syncthreads();

    for (int stride = blockDim.x / 2; stride > 0; stride >>= 1) {
        if (tid < stride && gid + stride < n) {
            sharedBounds[tid].expand(sharedBounds[tid + stride]);
        }
        __syncthreads();
    }

    if (tid == 0) {
        output[blockIdx.x] = sharedBounds[0];
    }
}
...
// Note: Bounding box computation is done on CPU using exact algorithm from lbvh_builder.hpp
// This ensures identical BVH quality to the original implementation

// ============================================================================
// Treelet Optimization (Karras & Aila)
// ============================================================================

#define TREELET_SIZE 7
#define MAX_TREELETS 65536

// Dynamic programming for optimal treelet topology
__device__ float computeOptimalTopology(
    const AABB* leafBounds, int leafMask, int subsetMask,
    AABB* aabbCache, float* costCache)
{
    // Base case: single leaf
    if (__popc(subsetMask) == 1) {
        int leafIdx = __ffs(subsetMask) - 1;
        return leafBounds[leafIdx].surfaceArea();
    }

    // Check cache
    if (costCache[subsetMask] >= 0.0f) {
        return costCache[subsetMask];
    }

    // Compute AABB for subset
    AABB subsetAABB;
    for (int i = 0; i < TREELET_SIZE; ++i) {
        if (subsetMask & (1 << i)) {
            subsetAABB.expand(leafBounds[i]);
        }
    }
    aabbCache[subsetMask] = subsetAABB;

    float minCost = 1e30f;

    // Try all partitionings (avoid complementary)
    int delta = (subsetMask - 1) & subsetMask;
    int partition = (-delta) & subsetMask;

    while (partition != 0) {
        int complement = subsetMask ^ partition;

        float leftCost = computeOptimalTopology(leafBounds, leafMask, partition, aabbCache, costCache);
        float rightCost = computeOptimalTopology(leafBounds, leafMask, complement, aabbCache, costCache);
        float cost = subsetAABB.surfaceArea() + leftCost + rightCost;

        minCost = fminf(minCost, cost);

        partition = (partition - delta) & subsetMask;
    }

    costCache[subsetMask] = minCost;
    return minCost;
}

// Treelet optimization kernel (one warp per treelet)
__global__ void optimizeTreeletsSAH(
    BVHNode* nodes, const AABB* triBounds,
    const uint32_t* primIndices, int* treeletRoots,
    int numTreelets, float rootArea)
{
    int treeletIdx = blockIdx.x;
    if (treeletIdx >= numTreelets) return;

    int warpLane = threadIdx.x % 32;
    int rootNode = treeletRoots[treeletIdx];

    // Shared memory for treelet optimization
    __shared__ AABB leafBounds[TREELET_SIZE];
    __shared__ AABB aabbCache[128];  // 2^7 = 128 subsets
    __shared__ float costCache[128];

    // Gather leaf nodes (collaborative with warp)
    // Simplified: Assume treelet has exactly TREELET_SIZE leaves
    // In practice, would need to traverse subtree

    // Compute optimal SAH cost using dynamic programming
    if (warpLane < 128) {
        costCache[warpLane] = -1.0f;
    }
    __syncwarp();

    if (warpLane == 0) {
        int leafMask = (1 << TREELET_SIZE) - 1;
        float optimalCost = computeOptimalTopology(leafBounds, leafMask, leafMask, aabbCache, costCache);

        // Compare with current cost
        float currentCost = nodes[rootNode].bounds.surfaceArea();

        // If improved, reconstruct treelet (simplified - full implementation needed)
        // This would involve rebuilding the subtree with optimal topology
    }
}
    ...
    float* costs, size_t n)
{
    ...
    float cost = 0.0f;
    if (node.isLeaf()) {
        cost = relativeArea * node.primCount;  // Intersection cost = 1.0
    } else {
        cost = relativeArea;  // Traversal cost = 1.0
    }
...
public:
    ...
    void build(const TriangleMesh& mesh) {
        ...
        // Step 2: Compute scene bounds on CPU (exact algorithm from lbvh_builder.hpp)
        std::vector<AABB> triBounds(n);
        CUDA_CHECK(cudaMemcpy(triBounds.data(), d_triBounds, aabbSize, cudaMemcpyDeviceToHost));
        ...
        // Step 4: Sort by Morton code (use thrust for simplicity)
        ...
        // Copy results back to host for CPU-side bounding box computation
        nodes.resize(2 * n - 1);
        primIndices.resize(n);
        std::vector<uint32_t> parents(2 * n - 1);
        ...
        CUDA_CHECK(cudaMemcpy(parents.data(), d_parents, (2 * n - 1) * sizeof(uint32_t), cudaMemcpyDeviceToHost));

        // Step 7: Compute bounding boxes bottom-up on CPU (exact algorithm from lbvh_builder.hpp)
...
int main(int argc, char** argv) {
    ...
    std::cout << "CUDA LBVH Builder with Karras & Aila Optimizations\n";
    std::cout << "==================================================\n";
    ...
    if (doExport) {
        std::cout << "\nExporting BVH...\n";
        exportBVHToOBJ("cuda_bvh_output.obj", builder.nodes, leavesOnly);
        std::cout << "Exported to: cuda_bvh_output.obj\n";
    }
