...
public:
    ...
    BVHResult convertToResult(int n) {
        BVHResult result;
        ...
        std::vector<uint32_t> sortedIndices(d_indices.begin(), d_indices.end());
        std::vector<LBVHNode> hostNodes(d_nodes.begin(), d_nodes.end());

        result.primIndices.assign(sortedIndices.begin(), sortedIndices.end());
        result.nodes.resize(hostNodes.size());

        const int leafBase = n - 1;
        for (int i = 0; i < static_cast<int>(hostNodes.size()); ++i) {
            const auto& src = hostNodes[i];
            auto& dst = result.nodes[i];
            
            dst.bounds.min = Vec3(src.bbox.min.x, src.bbox.min.y, src.bbox.min.z);
            dst.bounds.max = Vec3(src.bbox.max.x, src.bbox.max.y, src.bbox.max.z);

            const bool isLeaf = (src.leftChild & 0x80000000u) != 0;
            if (isLeaf) {
                // Leaf node
                dst.childCount = 0;
                dst.childOffset = 0;
                dst.primCount = 1;
                // primOffset is position in sorted array (leaf index - leafBase)
                dst.primOffset = static_cast<uint32_t>(i - leafBase);
            } else {
                // Internal node
                dst.childCount = 2;
                dst.childOffset = src.leftChild;
                dst.primCount = 0;
                dst.primOffset = src.rightChild; // Store right child like CPU LBVH
            }
            dst.axis = 0;
        }
...
LBVHBuilderCUDA::LBVHBuilderCUDA() : pImpl(new Impl()) {}

LBVHBuilderCUDA::~LBVHBuilderCUDA() { delete pImpl; }

BVHResult LBVHBuilderCUDA::build(const TriangleMesh& mesh) {
    ...
    // Upload data to GPU
    pImpl->prepareData(mesh);
    
    // Run GPU compute
    pImpl->runCompute(n);
    CUDA_CHECK(cudaDeviceSynchronize());

    // Convert to BVHResult
    return pImpl->convertToResult(n);
}
...
int main(int argc, char* argv[]) {
    ...
    std::cout << "Starting CUDA LBVH build..." << std::endl;
    ...
    std::cout << "Nodes: " << result.nodes.size() << std::endl;
    std::cout << "------------------------------------------------" << std::endl;

    if (!outputFile.empty()) {
        std::cout << "Exporting BVH to: " << outputFile << std::endl;
        exportBVHToOBJ(outputFile, result.nodes, leavesOnly);
    }
