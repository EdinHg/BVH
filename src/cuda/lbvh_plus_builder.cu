...
// ------------------------------------------------------------------
// REPLACEMENT: Greedy Agglomerative Clustering (Fast & Low Register Usage)
// ------------------------------------------------------------------

__device__ void optimizeTreeletAgglomerative(LBVHNode* nodes, int rootIdx) {
    // 1. Single-Pass Collection (Safe & Consistent)
    // We collect Leaves and Internal nodes in one traversal to ensure
    // we have the exact building blocks we touched.
    
    int leafIndices[MAX_TREELET_SIZE]; 
    AABB_cw activeBoxes[MAX_TREELET_SIZE]; 
    int activeNodeIdx[MAX_TREELET_SIZE];   
    int leafCount = 0;

    int collectedInternal[MAX_TREELET_SIZE]; // Storage for internal nodes we can reuse
    int internalCount = 0;
    ...
    while (top > 0) {
        ...
        if (n.leftChild & 0x80000000) {
            ...
            // Internal Node
            // If we haven't filled our quota of leaves, we act as if we are traversing down.
            // If we HAVE filled the quota, this node effectively becomes a "leaf" for the 
            // purposes of the treelet (a cut), but the logic below handles "real" leaves.
            // We just need to collect enough internal nodes to rebuild the tree.
    ...
    // We need at least 2 leaves to merge.
    if (leafCount < 2) return;

    // 2. Agglomerative Clustering (O(N^3) but N<=7, so it's tiny)
    // We rebuild the tree bottom-up.
    // We need (leafCount - 1) internal nodes to connect (leafCount) items.
    // We use collectedInternal[0] (which is rootIdx) for the LAST merge.
    // We use collectedInternal[1...k] for the intermediate merges.
    
    int currentActive = leafCount;
    int internalNodeUsed = 0; // Counter for intermediate nodes

    while (currentActive > 1) {
        // A. Find best pair to merge (Greedy Surface Area Heuristic)
        float bestArea = 1e30f; // Use huge float
        int bestI = -1, bestJ = -1;
        ...
        // B. Select which Internal Node index to reuse
        int targetInternalIdx;
        
        if (currentActive == 2) {
            // Final Merge: MUST use the original root to preserve tree connectivity
            targetInternalIdx = collectedInternal[0]; 
        } else {
            // Intermediate Merge: Use one of the collected internal nodes.
            // We skip index 0 (Root).
            // We are guaranteed to have enough nodes because a binary tree of L leaves has L-1 internal nodes.
            // We simply consume them linearly from index 1.
            int availIndex = 1 + internalNodeUsed;
            
            // Safety Check: In case of bizarre topology, don't crash.
            if (availIndex < internalCount) {
                targetInternalIdx = collectedInternal[availIndex];
            } else {
                // Fallback: This should theoretically not happen in a valid binary tree,
                // but if it does, we can't optimize further. Abort changes? 
                // Hard to abort halfway. We just reuse the root (bad topology but safe memory).
                targetInternalIdx = collectedInternal[0]; 
            }
            internalNodeUsed++;
        }

        // C. Update the Node in Global Memory
        ...
        // D. Update Parent Pointers
        // (Critical: The children need to know who their new dad is)
        nodes[activeNodeIdx[bestI]].parent = targetInternalIdx;
        nodes[activeNodeIdx[bestJ]].parent = targetInternalIdx;

        // E. Compact the Active List
        // Slot 'bestI' becomes the new merged node
        activeBoxes[bestI] = n.bbox;
        activeNodeIdx[bestI] = targetInternalIdx;

        // Slot 'bestJ' is removed by overwriting it with the last element
...
__global__ void kOptimizeMaximalTreelets(LBVHNode* nodes, int* subtreeSizes, int numInternalNodes) {
    ...
    if (i == 0) {
        ...
        // It is maximal if the parent is too big to be optimized
    ...
    if (isMaximal) {
        optimizeTreeletAgglomerative(nodes, i);#include "lbvh_plus_builder.cuh"
#include <cuda_runtime.h>
#include <device_launch_parameters.h>
#include <thrust/sort.h>
#include <thrust/reduce.h>
...
__global__ void kOptimizeMaximalTreelets(LBVHNode* nodes, int* subtreeSizes, int numInternalNodes) {
    ...
    // Strict bounds: Only optimize small clusters.
    // If < 4, not much to optimize. If > 7, too expensive/complex.
    if (mySize < 4 || mySize > MAX_TREELET_SIZE) return; 
    
    // Check if this is a "Maximal" treelet root
    bool isMaximal = false;
    if (i == 0) {
        isMaximal = true;
    } else {
